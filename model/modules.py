    ...
    def forward(self, inputs, emotions):
        ...
        if inputs is not None:
            if not self.training:
                assert emotions is None # use either reference audio or emotion
        ...
        if emotion_embed_hard is not None:
            emotion_embed_hard = emotion_embed_hard.squeeze()
        if emotion_embed_soft is not None:
            emotion_embed_soft = emotion_embed_soft.squeeze()
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        self.encoder_type = model_config["block_type"]
        E = model_config[self.encoder_type]["encoder_hidden"] 
...
class SpeakerClassifier(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        self.encoder_type = model_config["block_type"]
        self.hidden_size = model_config[self.encoder_type]["encoder_hidden"]
...
class StyleLoss(nn.Module):
    ...
    def gram_matrix(self, input):
        ...
        return torch.bmm(input, input.transpose(1, 2)) 

    def forward(self, input, target):
        assert len(input.shape) == 2
        input = self.gram_matrix(input)
        target = self.gram_matrix(target)
        return -1 * F.mse_loss(input, target)   # loss is negative because we want to maximize the loss
...
class OrthogonalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        ort_loss = torch.bmm(input, target.transpose(1, 2))  
